...
namespace CrtProduccion.entidades
{
  ...
  #region Métodos y funciones
        ...
        public int crearDatos()
        {
           fld_idVehiculo = 0;
            ...
            if (datamanager.ConexionAbrir())
            {
                ...
                SqlCommand cmd = new SqlCommand("Insert into Vehiculo(Ficha,Descripcion,idMarca,Modelo,idTipoVehiculo,placa,ano,chasis)" +
                                                " output INSERTED.idVehiculo" +
                                                " Values(@Ficha,@Descripcion,@idMarca,@Modelo,@idTipoVehiculo,@Placa,@ano,@chasis)", datamanager.ConexionSQL);
        ...
        public bool leerDatos(SqlDataReader dr, bool asignar)
        {
            ...
            if (dr.Read())
            {
                ...
                if (asignar)
                {
                    ...
                    fld_idVehiculo = Convert.ToByte((byte)dr["idTipoVehiculo"]);
        ...
        {
            var dr = datamanager.ConsultaLeer("select idVehiculo, Ficha,Descripcion,idMarca,Modelo,idTipoVehiculo,Placa,ano,chasis,idColor,idLlantas,idEstado,seguroVence,ultMantenim,kilometraje" +
                                               " from Vehiculo" +
        ...
        public bool buscar(int idVehiculo, bool asignar)
        {
            var dr = datamanager.ConsultaLeer("select idVehiculo, Ficha,Descripcion,idMarca,Modelo,idTipoVehiculo,Placa,ano,chasis,idColor,idLlantas,idEstado,seguroVence,ultMantenim,kilometraje" +
                                               " from Vehiculo" +
                                               " where idVehiculo = " + idVehiculo.ToString());
            return leerDatos(dr, asignar);
        }
        ...
        {
            var dr = datamanager.ConsultaLeer("select top 1 idVehiculo,Ficha, Descripcion,idMarca,Modelo,idTipoVehiculo,Placa,ano,chasis,idColor,idLlantas,idEstado,seguroVence,ultMantenim,kilometraje" +
                                               " from Vehiculo" +
